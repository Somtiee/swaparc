...
import { getHealthyArcProvider } from "./arcRpc.js";
...
export async function maintainRecurringRelayerGas(provider, relayerAddress) {
  ...
  const shortfall = targetWei - current + marginWei;
 ...
 */
async function ensureRelayerGasBudget(provider, relayerWallet, requiredWei = 0n) {
  ...
  const req = typeof requiredWei === "bigint" ? requiredWei : BigInt(requiredWei || 0);
  const needWei = req > minFloorWei ? req : minFloorWei;
  const targetWei = needWei + marginWei > healthyTargetWei ? needWei + marginWei : healthyTargetWei;
...
export async function executeRecurringPrivpayDeposit(schedule) {
  ...
  if (useAutomation) {
    ...
    try {
      ...
      if (auth?.active) {
        ...
        const maxPeriod = BigInt(auth.maxAmountPerPeriod || 0);
        if (maxPeriod > 0n && spent + amountUnits > maxPeriod) {
          return {
  ...
  const depGasForBudget = depositGasEstimate ?? 2_400_000n;
  const feeGasForBudget = 120_000n; // relayer-sent transferFrom usage-fee pull
  const requiredWei = ((depGasForBudget + feeGasForBudget) * gasPrice * 13n) / 10n;

  const gasTopUp = await ensureRelayerGasBudget(provider, relayer, requiredWei);

  const depositOverrides = { gasPrice };
  if (depositGasEstimate) {
    depositOverrides.gasLimit = (depositGasEstimate * 13n) / 10n;
  }
  ...
  const lockResult = await withRelayerTxLock(async () => {
    ...
    try {
      ...
      await ensureRelayerGasBudget(provider, relayer, (requiredWei * 3n) / 2n).catch(
        () => {}
      );
      try {
        sent = await trySend();

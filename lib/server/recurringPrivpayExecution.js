...
async function ensureRelayerGasBudget(provider, relayerWallet) {
  ...
  /** Minimum ARC to submit fee pull + pool deposit; below this we must top up or fail. */
  const minExecutionWei = parseArcAmountOrDefault(
    process.env.RECURRING_RELAYER_MIN_EXECUTION_ARC,
    "0.005"
  );
  /** Optional comfort buffer — top-up only when below this AND execution would still succeed after. */
  const comfortTargetWei = parseArcAmountOrDefault(
  ...
  // Relayer can execute now — do not block recurring on optional gas maintenance.
  ...
  if (!sponsorKeys.length) {
    throw new Error(
      `Recurring relayer is out of ARC gas (${ethers.formatEther(currentBalance)} ARC). Set RECURRING_RELAYER_GAS_SPONSOR_PRIVATE_KEY or fund relayer ${relayerAddress}.`
    );
  }
  ...
  for (const sponsorKey of sponsorKeys) {
    ...
    if (currentBalance + actualSend < minExecutionWei) {
      lastError = new Error(
        `Recurring gas sponsor ${sponsorAddress.slice(0, 10)}… cannot cover relayer gas shortfall (has ${ethers.formatEther(sponsorBalance)} ARC).`
      );
      continue;
    }

...
async function ensureRelayerGasBudget(provider, relayerWallet) {
  ...
  /** Below this balance we top up — must cover heavy txs (fee pull + pool deposit), not just a tiny buffer. */
  ...
  const sponsorKey = String(
    process.env.RECURRING_RELAYER_GAS_SPONSOR_PRIVATE_KEY ||
      process.env.ARC_DEPLOYER_PRIVATE_KEY ||
      ""
  ).trim();
  if (!/^0x[0-9a-fA-F]{64}$/.test(sponsorKey)) {
    throw new Error(
      "Recurring automation relayer is out of ARC gas and no sponsor key is configured. Set RECURRING_RELAYER_GAS_SPONSOR_PRIVATE_KEY (or ARC_DEPLOYER_PRIVATE_KEY)."
    );
  }
  ...
  const sendWei =
    shortfall > topUpArcWei
      ? shortfall + parseArcAmountOrDefault(process.env.RECURRING_RELAYER_TOPUP_MARGIN_ARC, "0.02")
      : topUpArcWei;

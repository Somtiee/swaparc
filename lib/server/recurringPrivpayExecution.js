...
async function ensureRelayerGasBudget(provider, relayerWallet) {
  ...
  /** Below this balance we consider topping up — keep modest; relayer txs rarely need >0.05 ARC. */
  const targetMinArcWei = parseArcAmountOrDefault(
  ...
  /** If relayer is within this much of target, skip sponsor transfer (avoids false failures). */
  const skipToleranceWei = parseArcAmountOrDefault(
    process.env.RECURRING_RELAYER_TOPUP_SKIP_TOLERANCE_ARC,
    "0.02"
  );

  const currentBalance = await provider.getBalance(relayerAddress);
  if (currentBalance >= targetMinArcWei) {
    return { funded: false, relayerBalanceWei: currentBalance.toString() };
  }

  const shortfall = targetMinArcWei - currentBalance;
  if (shortfall <= skipToleranceWei) {
    return {
      funded: false,
      relayerBalanceWei: currentBalance.toString(),
      skippedTopUp: true,
    };
  }
  ...
  if (!sponsorKeys.length) {
    throw new Error(
      "Recurring automation relayer is out of ARC gas and no sponsor key is configured. Set RECURRING_RELAYER_GAS_SPONSOR_PRIVATE_KEY (or ARC_DEPLOYER_PRIVATE_KEY / MY_PK)."
    );
  }

  // Send only what is needed (+ margin), capped at the configured top-up batch size.
  ...
  for (const sponsorKey of sponsorKeys) {
    ...
    if (sponsorBalance < sendWei) {
      lastError = new Error(
        `Recurring gas sponsor ${sponsorAddress.slice(0, 10)}… balance too low (${ethers.formatEther(sponsorBalance)} ARC; need ${ethers.formatEther(sendWei)}).`
      );
      continue;
    }
  ...
  throw (
    lastError ||
    new Error(
      "Recurring gas sponsor wallet balance is too low to top up relayer ARC gas."
    )
  );
}

...
function userFacingGasError(message) {
  ...
  if (/gas sponsor|intrinsic transaction cost|insufficient funds/i.test(msg)) {
    return "Autopay relayer is low on ARC gas. Tap Fund autopay gas on this bill (or toggle Recurring off → on).";
  }
...
async function ensureRelayerGasBudget(provider, relayerWallet) {
  ...
  if (!sponsorKeys.length) {
    throw new Error(userFacingGasError(
      `Recurring relayer needs ${ethers.formatEther(minExecutionWei)} ARC gas (has ${ethers.formatEther(afterMaintain)}). Tap Fund autopay gas on this bill.`
    ));
  }
  ...
  for (const sponsorKey of sponsorKeys) {
    ...
    if (affordable <= 0n) {
      lastError = new Error(userFacingGasError("Autopay relayer is low on ARC gas."));
      continue;
    }

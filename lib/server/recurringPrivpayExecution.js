...
function userFacingGasError(message) {
  ...
  if (/gas sponsor|intrinsic transaction cost|insufficient funds/i.test(msg)) {
    return "Autopay relayer is low on native USDC (gas). Tap Fund autopay gas on this bill (or toggle Recurring off → on).";
  }
...
export async function maintainRecurringRelayerGas(provider, relayerAddress) {
  ...
  if (current >= minExecutionWei) {
    return { ok: true, skipped: true, balanceWei: current.toString() };
  }

  const shortfall = targetWei > current ? targetWei - current : minExecutionWei - current;
  ...
  for (const sponsorKey of parseSponsorPrivateKeys()) {
    ...
    if ((await sponsor.getAddress()).toLowerCase() === relayerAddress.toLowerCase()) {
      continue;
    }
    const sponsorBalance = await provider.getBalance(await sponsor.getAddress());
    if (sponsorBalance <= sendWei) continue;
    const topUpTx = await sponsor.sendTransaction({ to: relayerAddress, value: sendWei });
...
async function ensureRelayerGasBudget(provider, relayerWallet) {
  ...
  /** Must cover fee pull + pool deposit on Arc testnet (native USDC gas). */
  const minExecutionWei = parseArcAmountOrDefault(
    process.env.RECURRING_RELAYER_MIN_EXECUTION_ARC,
    "0.04"
  );
  ...
  await maintainRecurringRelayerGas(provider, relayerAddress);
  const afterMaintain = await provider.getBalance(relayerAddress);
  if (afterMaintain >= minExecutionWei) {
    return { funded: true, relayerBalanceWei: afterMaintain.toString(), viaMaintain: true };
  }

  const shortfall = minExecutionWei - afterMaintain;
  let sendWei = shortfall + marginWei;
  if (sendWei > topUpArcWei) sendWei = topUpArcWei;

  const sponsorKeys = parseSponsorPrivateKeys();
  if (!sponsorKeys.length) {
    throw new Error(userFacingGasError(
      `Recurring relayer needs ${formatNativeGasUsdc(minExecutionWei)} gas (has ${formatNativeGasUsdc(afterMaintain)}). Tap Fund autopay gas on this bill.`
    ));
  }
  ...
  for (const sponsorKey of sponsorKeys) {
    ...
    if (affordable <= 0n) {
      lastError = new Error(userFacingGasError("Autopay relayer is low on native USDC (gas)."));
      continue;
    }
    ...
    try {
      ...
      lastError = new Error(userFacingGasError(
        `Relayer still below gas minimum after top-up (${ethers.formatEther(newBalance)} ARC). Tap Fund autopay gas on this bill.`
      ));
    } catch (err) {
  ...
  throw (
    lastError ||
    new Error(userFacingGasError("Recurring relayer is out of ARC gas. Tap Fund autopay gas on this bill."))
  );
}

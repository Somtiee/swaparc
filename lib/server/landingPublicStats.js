...
/** Persist JSON for CDN / Blob (optional). Returns public URL when Blob upload succeeds. */
export async function publishLandingPublicStats(payload) {
  ...
  if (token) {
    try {
      ...
      const result = await put("landing-network.json", JSON.stringify(payload), {
        access: "public",
      ...
      console.warn("[landing-public-stats] Blob upload failed:", err?.message || err);
    }
  }

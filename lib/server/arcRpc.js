...
const ARC_CHAIN_ID = ARC_CHAIN_ID_DEC;
...
/**
 * Ordered RPCs for Arc (testnet today, mainnet via ARC_* env — see
 * lib/arcNetwork.js). Used by Swap / Pools / PrivPay / Profile / recurring:
 * 1) public Arc RPC
 * 2) free dRPC
...
/**
 * Run `fn(provider, url)` against Arc RPCs until one succeeds.
 * Free endpoints (public + dRPC) are raced; Alchemy is last resort only.
 */
export async function withArcRpc(fn, label = "rpc", perAttemptMs = 6000) {
  const urls = getArcRpcUrls();
  const alchemy = String(
    process.env.ALCHEMY_ARC_RPC_URL || process.env.VITE_ALCHEMY_ARC_RPC_URL || ""
  ).trim();
  const freeUrls = urls.filter((u) => u && u !== alchemy);
  const paidUrls = urls.filter((u) => u && u === alchemy);
  let lastErr = null;
  ...
  if (freeUrls.length > 0) {
    try {
      ...
      for (const err of errs) {
        ...
        console.warn(
          `[arcRpc] ${label} free RPC fail: ${String(err?.message || err).slice(0, 160)}`
        );
      }
    }
  }
  ...
  for (const url of paidUrls) {
    ...
    try {
      ...
      console.warn(
        `[arcRpc] ${label} failed on Alchemy: ${String(err?.message || err).slice(0, 160)}`
      );
    } finally {
      if (timer) clearTimeout(timer);
    }
  ...
  throw lastErr || new Error(`${label} failed on all Arc RPCs`);
}
